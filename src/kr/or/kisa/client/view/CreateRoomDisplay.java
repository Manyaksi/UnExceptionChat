...
class CreateRoomDisplay extends JDialog implements ActionListener, ItemListener {
	...
	public CreateRoomDisplay(ChatUI ui) {
		super(ui, true);
		this.ui = ui;

		// main = frame;
		// ui = new ChatUI_Test();
		// panel = new WaitRoomPanel(ui);
		setTitle("대화방 개설");
		isRock = 0;
		roomMaxUser = 2;
		str_password = "0";
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == ok) {
			if (tf.getText().equals("")) {
				JOptionPane.showMessageDialog(main, "방제목을 입력하세요", "대화방 개설.",
				...
				if (isRock == 1
					...
					// public Room(String roomName, int roomMaxUser, boolean
					// isRock, String password, String roomOwner, MessageHandler
					// client){

					String message = Message.REQUEST_CREATROOM
							+ Message.DELIMETER + roomName + "," + roomMaxUser
							+ "," + isRock + "," + str_password;
					ui.createChatRoom(message);
					dispose();
				} else if (isRock == 1
						&& new String(password.getPassword()).equals("")) {
